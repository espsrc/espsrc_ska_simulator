...
def distance_matrix_in_arcsec(
    wcs, shape, ra_deg, dec_deg, radius_arcsec, pixscale=1 * u.arcsec
):
    """
    ...
    try:
        ...
        wcs.wcs.cdelt = np.array(
            [-1 * pixscale.to(u.deg).value, pixscale.to(u.deg).value]
        )  # degrees/pixel
        ...
        radius_pix = (
            radius_arcsec.to(u.deg).value / pixscale_deg
...
if __name__ == "__main__":
    try:
        ...
        matriz = distance_matrix_in_arcsec(
            wcs, shape, 150.0, 2.0, r_out
        )  # radio 10 arcsec
        matriz = setmask(
            matriz, r_in.to(u.arcsec).value, r_out.to(u.arcsec).value, np.nan
        )

        mymask = np.isfinite(matriz)  # Mask for finite values
        mean_value = (np.nanmax(matriz) - np.nanmin(matriz)) / 2
        matriz[mymask] = -1 * np.abs(
            matriz[mymask] - mean_value
        )  # Normalizar a la mitad del rango
