...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="*",
            help="List of input files (not used in this test)",
        )
        parser.add_argument(
            "--components",
            type=int,
            default=None,
            help="Number of components for source modeling",
        )
        parser.add_argument(
            "--snr-threshold",
            type=float,
            default=3.0,
            help="SNR threshold for source detection",
        )
        parser.add_argument(
            "--rms",
            type=float,
            default=0.0,
            help="RMS noise level in Jy/beam (if 0, it will be estimated from the image)",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="File with source list to overlay",
        )
        parser.add_argument(
            "--author",
            type=str,
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help='Beam parameters to use if not in FITS header (e.g., "10arcsec,8arcsec,30deg")',
        )
        parser.add_argument(
            "--output", type=str, default=None, help="Output FITS file name"
        )
        ...
        if args.beam != "":
            try:
                ...
                print(
                    f"Using user-defined beam: {default_beam.major}, {default_beam.minor}, {default_beam.pa}"
                )
        ...
        for idx, path in enumerate(args.files):
            ...
            if args.components is not None:
                ...
                sorted_indices = flat_data.argsort()[
                    ::-1
            ...
            try:
                image.name = (
                    ("_").join(path.split("/")[-2].split("_")[2:]).replace("_", " ")
                )
            except Exception:
                image.name = "model"
            if default_beam is None:
                default_beam = Beam(
                    image.pixelarea**0.5 * 5, image.pixelarea**0.5 * 5, 0 * u.deg
                )
            ...
            print(
                f"\tBeam: {beam.major.to(u.arcsec):.4f} x {beam.minor.to(u.arcsec):.4f} ({beam.pa:.1f})"
            )
            ...
            temp_path = (
                f"temp_image{suffix}.fits"
                if args.output is None
                else args.output.replace(".fits", "_temp.fits")
            )
            ...
            d = Dendrogram.compute(
                ...
                min_delta=rms,
                min_npix=pixels_in_beam,
            )
            # p = d.plotter()
            leaves = d.leaves
            ax = fig.add_subplot(
                1, len(args.files), idx + 1, projection=image.wcs.celestial
            )
            # ax.imshow(image.data2d, origin='lower', cmap='viridis', interpolation='nearest',vmin=0, vmax=np.percentile(image.data2d, 99))
            image.draw(
                img=ax,
                title=f"{image.name} ({len(leaves)} sources)",
                exp=0.5,
                cmapstr="viridis",
                colorbar=True,
                show_beam=True,
            ...
            metadata["spatial_scale"] = (
                math.sqrt(pixel_area.to(u.arcsec**2).value) * u.arcsec
            )
            ...
            for leaf in d.all_structures:
                ...
                src = Source(
                    ra=ra * u.deg,
                    dec=dec * u.deg,
                    obj_id=f"SRC{leaf.idx:03d}",
                    I=stat.flux,
                    spec_index=0,
                    ref_freq=image.restfreq(),
                    major_axis=stat.major_sigma,
                    minor_axis=stat.minor_sigma,
                    pa=stat.position_angle,
                    isl_rms=rms_src,
                    resolved=n_pix > pixels_in_beam,
                )
                sources.append(src)
            ...
            if len(sources) > 0:
                ...
                ref_freq_list = np.array(
                    [source.ref_freq.to(u.MHz).value for source in sources]
                )
                rm_list = np.array(
                    [source.rot_meas.to(u.rad / u.m**2).value for source in sources]
                )
                ...
                isl_rms_list = np.array(
                    [source.isl_rms.to(u.Jy).value for source in sources]
                )
                ...
                cols = [
                    ...
                    fits.Column(
                        name="REFFREQ", format="E", unit="MHz", array=ref_freq_list
                    ),
                    fits.Column(name="RM", format="E", unit="rad/m^2", array=rm_list),
                    fits.Column(
                        name="MAJ", format="E", unit="arcsec", array=major_axis_list
                    ),
                    fits.Column(
                        name="MIN", format="E", unit="arcsec", array=minor_axis_list
                    ),
                    fits.Column(name="PA", format="E", unit="deg", array=pa_list),
                    fits.Column(
                        name="RESOLVED", format="L", array=resolved_list
                    ),  # Placeholder column
                    fits.Column(
                        name="ISL_RMS", format="E", unit="Jy/beam", array=isl_rms_list
                    ),  # Placeholder column
                ]
                ...
                path_fits = (
                    f"test_astro_sources{suffix}.fits"
                    if args.output is None
                    else args.output
                )
        ...
        plt.tight_layout()
        plot_path = (
            f"plot_dendro{suffix}.png"
            if args.output is None
            else args.output.replace(".fits", ".png")
        )
