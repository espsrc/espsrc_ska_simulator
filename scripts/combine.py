...
class Combination:
    def __init__(self, img1, img2, *imgs):
        if not isinstance(img1, Image) and os.path.exists(img1):
            img1 = Image(img1)
        if not isinstance(img2, Image) and os.path.exists(img2):
            img2 = Image(img2)
        imgs_list = []
        for img in list(imgs):
            if not isinstance(img, Image) and os.path.exists(img):
                imgs_list.append(Image(img))
        imgs = imgs_list
        
        try:
            imgs_list= [img1, img2] + list(imgs)
            imgs_list = sorted(imgs_list, key= lambda x:x.pixelarea, reverse=True)
            self.bg = imgs_list[0]
            self.fg = imgs_list[1:]
            self.row_min = []
            self.row_max = []
            ...
            t0 = time.time()
#             for fg in self.fg:
#                 mylog ("Combining %s (bg image) with %s (fg image)..." % (os.path.basename(self.bg.path),os.path.basename(fg.path)))
        except Exception as e:
            mylog (show_exc(e))
            return None

    def is_possible(self, distance=0.5, verbose=False):
        inter_center = self.fg[0].get_center(frame=FK5)
        (x,y) = self.bg.coords2pix(inter_center)
        if verbose:
            mylog("Pixels: ", x,y)
        try:
            aux=self.bg.data[int(y),int(x)]
            if verbose:
                mylog("Value in ({},{}): {}", x,y,aux)
            return True
        except Exception as e:
            if verbose:
                mylog (show_exc(e))
    ...
    #In Combination
    def reprojection(self, img = None, to_pixel=False, verbose=False):
        """ Return: list of duplas; first element is single_dish reprojected on interferometer images; second element is interferometer image """
        results = []
        if img is None:
            img = self.fg
        else:
        ...
        for if_img in img:
            try:
                ...
                hdu.header['NAXIS'] = 2
                hdu.header['NAXIS1'] = if_img.data.shape[0]
                hdu.header['NAXIS2'] = if_img.data.shape[1]
                hdu.header['EXTEND'] = True
                hdu.header['CRVAL1'] = if_img.header['CRVAL1']
                hdu.header['CRVAL2'] = if_img.header['CRVAL2']
                hdu.header['CDELT1'] = if_img.header['CDELT1']
                hdu.header['CDELT2'] = if_img.header['CDELT2']
                hdu.header['CUNIT1'] = if_img.step[0].unit.name
                hdu.header['CUNIT2'] = if_img.step[1].unit.name
                try:
                    hdu.header['BMAJ'] = self.bg.header['BMAJ']
                    hdu.header['BMIN'] = self.bg.header['BMIN']
                except:
                    if 'BGPSVERS' in self.bg.header.keys():
                        hdu.header['BMAJ'] = BGPS.bmaj.value
                        hdu.header['BMIN'] = BGPS.bmin.value

                hdu.header['CRPIX1'] = if_img.header['CRPIX1']
                hdu.header['CRPIX2'] = if_img.header['CRPIX2']
                hdu.header['CTYPE1'] = if_img.header['CTYPE1'] 
                hdu.header['CTYPE2'] = if_img.header['CTYPE2'] 
                hdu.header['RESTFRQ'] = self.bg.restfreq().to(u.Hz).value
                try:
                    hdu.header['RADESYS'] = if_img.header['RADESYS']
                except:
                    hdu.header['RADESYS'] = 'ICRS'
                hdu.header['BUNIT'] = 'Jy/beam'
                try:
                    hdu.header['BPA'] = self.bg.header["BPA"]
                except:
                    hdu.header['BPA'] = if_img.header["BPA"]
                hdu.header['AUTHOR'] ='IMGCombine by d.diaz@irya.unam.mx'
                try:
                    hdu.header['OBJECT'] = if_img.header['OBJECT']
                except:
                    hdu.header['OBJECT'] = 'UNDEFINED'
#                 try:
#                     hdu.header['RESTFRQ'] = self.bg.header['REFFREQ']
#                 except:
#                     hdu.header['RESTFRQ'] = 'UNDEFINED'
                obj = WCS(hdu.header)
                try:
                    array, footprint = rp.reproject_interp(self.bg.fits, obj, if_img.data.shape)
                    #array, footprint = rp.reproject_exact(self.bg.fits, obj, if_img.data.shape)
                except:
                    array, footprint = rp.reproject_interp((self.bg.data,self.bg.header), obj, if_img.data.shape)
                    #array, footprint = rp.reproject_exact((self.bg.data,self.bg.header), obj, if_img.data.shape)
                if to_pixel:
                    singledish_im = array / (self.bg.omega_beam() / if_img.omega_pix()).value #Convert Jy/beam to Jy/px
                    a = Image(data=singledish_im,header=hdu.header)
                    a.header['BUNIT'] = 'Jy/px'
                else:
                    a = Image(data=array,header=hdu.header)
                    a.header['BUNIT'] = 'Jy/beam'
                a.name = if_img.name
                a.header = hdu.header
                if hasattr(if_img, 'frame'):
                    a.frame = if_img.frame
                else:
                    a.frame = FK5
                a.ratiopixel = (self.bg.pixelarea / if_img.pixelarea).value
                a.pixelarea = if_img.pixelarea
                results.append((a, if_img))
            except Exception as e:
    ...
    def combine_feathering(self, lowresscalefactor=1., highresscalefactor=1., tofits=False, fitspath=".", match = False, pb_path = None, replaceSD=0.0, deconvSD=False, alpha_spec=3.5):
        sd_img = self.bg
        sd_beam = sd_img.get_beam()
        for item in self.fg:
            alma_img = item

            freq_factor = (alma_img.restfreq().value / sd_img.restfreq().value)**(alpha_spec) 
            singledish_im = (sd_img.data*freq_factor) / (sd_img.omega_beam() / sd_img.omega_pix()).value

            alma_beam = alma_img.get_beam()
            im_interferometered = alma_img.data / (alma_img.omega_beam() / alma_img.omega_pix()).value # Convert Jy/beam to Jy/px

            #singledish_kernel = convolution.Gaussian2DKernel(sd_beam.major.to(u.arcsec).value)
            #singledish_kernel_fft = np.fft.fft2(singledish_kernel)

            pixscale = (alma_img.pixelarea**0.5).to(u.arcsec).value
            lowresfwhm = sd_beam.major.to(u.arcsec).value
            nax1,nax2 = sd_img.data.shape
            im_hi = im_interferometered
            im_low = singledish_im

            kfft, ikfft = feather_kernel(nax1, nax2, lowresfwhm, pixscale)
            if replaceSD == 0.0:
                replaceSD = False

            fftsum, combo = fftmerge(kfft, ikfft, im_hi * highresscalefactor, im_low * lowresscalefactor, lowpassfilterSD=not(replaceSD), replace_hires=replaceSD, deconvSD=deconvSD)

            prefix = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
            ### TEST ##
            mask_nan = np.isnan(item.data)
            combo.real[mask_nan] = np.nan
            ### END TEST ##
            product = Image(data=combo.real * (alma_img.omega_beam() / alma_img.omega_pix()).value, header=item.header)
            #product = Image(data=combo.real, header=item.header)
            product.header['LO_SCALE'] = lowresscalefactor
            product.header['HI_SCALE'] = highresscalefactor
            product.header['OBJECT']  = alma_img.header['OBJECT']
            product.header['RESTFRQ'] = alma_img.restfreq().to(u.Hz).value
            if tofits:
                product.tofits(os.path.join(fitspath, '%s_lo%.3f_hi%.3f.fits' % (prefix, lowresscalefactor, highresscalefactor)))
            return product, sd_img

    def compare(self, png_path = None, SAS = 9*u.arcsec, LAS=16*u.arcsec, weights=None, crop=None):
        #assert LAS > SAS
        mgps = self.bg
        alma = self.fg[0]
        region_name = alma.name
        mgps.data = mgps.data / (self.bg.omega_beam() / mgps.omega_pix()).value #Convert Jy/beam to Jy/px
        alma.data = alma.data / (alma.omega_beam()/alma.omega_pix()).value #Convert Jy/beam to Jy/px

        prefix = time.time()

        ftemp = '{0}_temp_{0}.fits'.format(int(prefix))
        mgps.tofits(ftemp)
        lowresfwhm = mgps.get_beam().major.to(u.arcsec)
        ...
        if weights is not None:
            if not weights.shape == im_hi.shape:
                raise ValueError("weights must be an array with the same shape as the high-res data.")
        else:
            weights = 1.

        (nax2,nax1) = mgps.data.shape
        pixscale = (alma.pixelarea**0.5).to(u.deg).value
        ...
        if SAS < LAS:
            pl.clf()
#         pl.suptitle("{0} S.A.S: {1} - L.A.S.: {2}".format(alma.name.upper(),SAS,LAS))
            ax0 = pl.subplot(2,2,1)
#         pl.plot(np.abs(fft_hi)[mask], np.abs(fft_lo_deconvolved)[mask], '.')

#         ax1 = ax0.twiny()
#         srt = np.argsort(np.uniqueangscales.to(u.arcsec).value[mask])
#         srt = [f'{index:.2f}' for index in srt]
#         ax1.set_xticklabels(srt)


            pl.ylim(ymin=0, ymax=5.)
            pl.plot(angscales.to(u.arcsec)[mask].flatten().value,  np.abs(fft_hi[mask]).flatten() / np.abs(fft_lo_deconvolved[mask]).flatten(),'.')

            pl.grid()
            pl.xlabel("[arcsec]", fontsize='xx-large')
            pl.ylabel('Amplitude Ratio.\nHigh-resolution / Low-resolution', fontsize='xx-large')
            for label in (ax0.get_xticklabels() + ax0.get_yticklabels()):
                label.set_fontsize('xx-large')

            ax0 = pl.subplot(2,2,2)
            pl.hist(ratio[np.isfinite(ratio)], bins=30)
            pl.xlabel("Amplitude Ratio. High-resolution / Low-resolution", fontsize='xx-large')
            for label in (ax0.get_xticklabels() + ax0.get_yticklabels()):
                label.set_fontsize('xx-large')

            ax0 = pl.subplot(2,1,2)
            if crop is None:
                #srt = np.argsort(angscales.to(u.arcsec).value[~below_beamscale_plotting])
                l0=pl.loglog(angscales.to(u.arcsec).flatten().value, np.abs(fft_hi).flatten(), 'or', alpha=0.5, ms=1.5, label='High-res')
                l1=pl.loglog(angscales.to(u.arcsec).flatten().value, np.abs(fft_lo_deconvolved).flatten(), 'ob', alpha=0.5, ms=1.5, label='Low-res')
            else:
                mask = ((angscales.to(u.arcsec).flatten() >= crop[0]) & (angscales.to(u.arcsec).flatten() <= crop[1]))
                l0=pl.loglog(angscales.to(u.arcsec).flatten()[mask].value, np.abs(fft_hi).flatten()[mask], 'or', alpha=0.5, ms=1.5, label='High-res')
                l1=pl.loglog(angscales.to(u.arcsec).flatten()[mask].value, np.abs(fft_lo_deconvolved).flatten()[mask], 'ob', alpha=0.5, ms=1.5, label='Low-res')
                ax0.xaxis.set_minor_formatter(FormatStrFormatter("%.1f"))
                
            pl.xlabel('[arcsec]', fontsize='xx-large')
            pl.ylabel('FFT Ampl.', fontsize='xx-large')
            for label in (ax0.get_xticklabels() + ax0.get_yticklabels()):
                label.set_fontsize('xx-large')

            pl.legend() 
            #pl.legend([l0,l1],['High Res','Low-Res'])
            ylim = pl.gca().get_ylim()
            pl.vlines([LAS.to(u.arcsec).value], ylim[0], ylim[1], linestyle='-', color='r')
            pl.vlines([SAS.to(u.arcsec).value], ylim[0], ylim[1], linestyle='-', color='b')
            if SAS < LAS:
                pl.axvspan(SAS.to(u.arcsec).value,LAS.to(u.arcsec).value, color='grey', alpha=0.3)
                pl.title(f'Common angular scales [{SAS} - {LAS}]')
            else:
                pl.title('NO COMMONS SCALES')
            pl.grid()
            if not png_path:
                plt.show()
            else:
            ...
            pl.vlines([LAS.to(u.arcsec).value], ylim[0], ylim[1], linestyle='-', color='r')
            pl.vlines([SAS.to(u.arcsec).value], ylim[0], ylim[1], linestyle='-', color='b')
            pl.title(f'{region_name} - NO COMMONS SCALES')
            pl.grid()
            if not png_path:
                plt.show()
            else:
    ...
    def tofits(self, filename=None):
        ...
        hdu.header['NAXIS1'] = self.combination.size[0]
        hdu.header['NAXIS2'] = self.combination.size[1]
        hdu.header['EXTEND'] = False
        hdu.header['CRVAL1'] = np.min(self.axis[0])
        hdu.header['CRVAL2'] = np.max(self.axis[1])
        hdu.header['CDELT1'] = np.abs(self.axis[0,0] - self.axis[0,1])
        hdu.header['CDELT2'] = np.abs(self.axis[1,0] - self.axis[1,1])
        hdu.header['CRPIX1'] = 0
        hdu.header['CRPIX2'] = 0
    ...
    def scale_factor(self, SAS = 10*u.arcsec, LAS=16*u.arcsec, fit = None,weights=None, plot=False, verbose=True, nofit=False, alpha_spec=3.5):
        try:
            mgps = self.bg
            results = []
            if (mgps.get_beam().major <= SAS):
                for alma in self.fg:
                    mgps = self.bg
                    freq_factor = (alma.restfreq().value / mgps.restfreq().value)**(alpha_spec) 
                    mgps.data *= freq_factor

                    angscales_sd, fft_lo = mgps.to_fourier_space()
                    angscales_if, fft_hi = alma.to_fourier_space()
                    if hasattr(weights, 'shape'):
                        fft_hi *= (1-weights)
                        fft_lo *= weights
                    mask_ratio = (angscales_if >= SAS) & (angscales_if <= LAS)
                    (hi_lo_ratio, lo_hi_ratio) = (np.nanmedian(fft_hi[mask_ratio] / fft_lo[mask_ratio]), np.nanmedian(fft_lo[mask_ratio] / fft_hi[mask_ratio]))

                    results.append({'name':"{} vs {}".format(os.path.basename(alma.path), os.path.basename(mgps.path)), 'hi/lo':hi_lo_ratio, 'lo/hi':lo_hi_ratio, 'fft_hi':fft_hi, 'fft_lo':fft_lo, 'angscales':angscales_if})
                if len(results) == 1:
                    return (results[0])
            else:
                mylog("Your SAS is {} and beam is {}".format(SAS,mgps.get_beam().major.to(SAS.unit)), verbose=verbose)
                return {'hi_median':0., 'lo_median':0., 'hi/lo':0., 'lo/hi':0.}
        except Exception as e:
            print(show_exc(e))
            mylog (show_exc(e), verbose=verbose)
            return {'hi_median':0., 'lo_median':0., 'hi/lo':0., 'lo/hi':0.}

    def alignment(self, pb_path, pb_mask_value=0.2, mask_align=0.5, sigma=0., align_fits_path=None, to_pixel=False, verbose=True, both_pb=False):
        ''' 
            pb_path: Primary Beam Fits from interferometer 
            pb_mask_value: Set mask to pb.fits > pb_mask_value
            mask_align: 
        '''
        try:
            mylog ("\tReprojecting... ",  end=" ",flush=True)
            (sing_img, alma_img) = self.reprojection(to_pixel=to_pixel)
            mylog ("OK")

            if pb_path is not None and os.path.exists(pb_path):
                mylog ("\tLoading %s..." % pb_path,  end=" ",flush=True)
                pb_img = Image(pb_path)
                mylog ("OK")
            else:
                mylog ("\tPrimary Beam file does not exists. Generating fake Primaby Beam from interferometer...",  end=" ",flush=True)
                pb_img = Image(data=np.ones(alma_img.data.shape), header=alma_img.header)
                mylog ("OK")


            mylog ("\tCalculating beams... ",  end=" ",flush=True)
            try:
                alma_beam = Beam(alma_img.header['BMAJ'] * u.deg, alma_img.header['BMIN'] * u.deg, alma_img.header['BPA'] * u.deg)
                sing_beam = Beam(sing_img.header['BMAJ'] * u.deg, sing_img.header['BMIN'] * u.deg, sing_img.header['BPA'] * u.deg)
                pixscale = (alma_img.pixelarea**0.5).to(u.arcsec)
                aux_beam = rbu.deconvolve(sing_beam, alma_beam)
                aux_beam = Beam(aux_beam[0], aux_beam[1], aux_beam[2])

                mylog ("\tConvolving... ",  end=" ",flush=True)
                alma_conv = convolve_fft(alma_img.data, aux_beam.as_kernel(pixscale), allow_huge=True)

                mask_min = (pb_img.data < pb_mask_value) | (np.isnan(pb_img.data))
                alma_conv[mask_min] = np.nan

                if both_pb:
                    sing_img.data[mask_min] = np.nan

                mylog ("OK")
                mylog ("\tScaling and Stats... ",  end=" ",flush=True)
                conv_img = Image(data=alma_conv, header=sing_img.header)
                factor_scale = ((1/(conv_img.janskys_pixel() / alma_img.janskys_pixel())) * (alma_img.Npts() / conv_img.Npts()))
                conv_img.data *= factor_scale.value
                align_mask = None

                from image_registration import chi2_shift
                from image_registration.fft_tools import shift
                if mask_align > 0.0:
                    align_mask = (pb_img.data < mask_align) 
                    conv_img.data[align_mask] = np.nan
                else:
                    if sigma >= 1.0:
                        align_mask = (conv_img.data < sigma * conv_img.mad())  | (sing_img.data < sigma * sing_img.mad()) 
                        conv_img.data[align_mask] = np.nan
                if align_fits_path is not None and verbose:
                    conv_img.tofits(align_fits_path.replace('.fits','_conv.fits'))
                aux0 = np.copy(conv_img.data)
                aux1 = np.copy(sing_img.data)
                if align_mask is not None:
                    aux0[align_mask] = np.nan
                    aux1[align_mask] = np.nan  
                if align_fits_path is not None and verbose:
                    crop_img = Image(data=aux1, header=sing_img.header)
                    crop_img.tofits(align_fits_path.replace('.fits','_noalign.fits'))
                xoff, yoff, exoff, eyoff = chi2_shift(aux0, aux1) #, noise, return_error=True, upsample_factor='auto')
                center_before= sing_img.get_center()
                sing_img.header['CRVAL1'] += (-xoff)*sing_img.header['CDELT1']
                sing_img.header['CRVAL2'] += (-yoff)*sing_img.header['CDELT2']
                sing_img.header['ORGFILE'] = self.bg.path
                center_after = sing_img.get_center()
                gxoff = center_after.galactic.l - center_before.galactic.l
                gyoff = center_after.galactic.b - center_before.galactic.b
                self.bg.header['CRVAL1'] += (gxoff.to(u.deg).value)
                self.bg.header['CRVAL2'] += (gyoff.to(u.deg).value)
                ftemp = "%d.fits" % int(time.time())
                self.bg.tofits(ftemp)
                self.bg = Image(ftemp)
                (sing_img, alma_img) = self.reprojection(to_pixel=to_pixel)

                stats = {
                            'xoff_pix':xoff, 'yoff_pix':yoff, 'exoff_pix':exoff, 'eyoff_pix':eyoff, 
                            'xoff':xoff*sing_img.header['CDELT1']*u.Unit(sing_img.header['CUNIT1']),
                            'yoff':yoff*sing_img.header['CDELT2']*u.Unit(sing_img.header['CUNIT2']),
                            'exoff':exoff*sing_img.header['CDELT1']*u.Unit(sing_img.header['CUNIT1']),
                            'eyoff':eyoff*sing_img.header['CDELT2']*u.Unit(sing_img.header['CUNIT2']),
                            'angle':np.rad2deg(np.arctan(yoff/xoff))
                        }
                mylog ("OK")
            except Exception as e:
                mylog ("WARNING: We can not align the images. Original alignment.\n{}".format(show_exc(e)))
                xoff, yoff, exoff, eyoff = 0, 0, 0, 0
                stats = {
                            'xoff_pix':xoff, 'yoff_pix':yoff, 'exoff_pix':exoff, 'eyoff_pix':eyoff, 
                            'xoff':xoff*sing_img.header['CDELT1']*u.Unit(sing_img.header['CUNIT1']),
                            'yoff':yoff*sing_img.header['CDELT2']*u.Unit(sing_img.header['CUNIT2']),
            ...
            if align_fits_path:
                mylog ("\tSaving to %s..." % align_fits_path, end=" ",flush=True)
                sing_img.tofits(align_fits_path)
                mylog ("OK")
            ...
            return sing_img, stats
        except Exception as e:
            mylog (show_exc(e))
            return self.bg, {'error':show_exc(e)}
