...
def printlog(msg, t0=None):
    ...
    if t0:
        ...
        print(
            f"[{datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')}] {msg}"
        )
...
def calculate_lst(longitude, latitude, observation_time_local):
    ...
    location = EarthLocation(
        lat=latitude * u.deg, lon=longitude * u.deg, height=300 * u.m
    )

    # Convert local time to UTC
    observation_time = (
        Time(observation_time_local, format="iso", scale="utc")
        - (longitude / 15.0) * u.hour
    )
...
if __name__ == "__main__":
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Generate a sky model with sources. If the user does not provide any parameters, the script will generate a random sky model with 10 sources."
        )
        parser.add_argument(
            "--cfg", help="Path to the config file", type=str, default=None
        )
        parser.add_argument(
            "--out",
            help="Path to the output file (if this parameter is not provided, then filenames based on the date will be generated)",
            type=str,
            default=".png",
        )
        parser.add_argument(
            "--N_srcs", help="Generate N random sources", type=int, default=10
        )
        parser.add_argument("--rms", help="RMS noise", type=float, default=0.0)
        parser.add_argument(
            "--tofits", help="Save to fits", action="store_true", default=False
        )
        parser.add_argument(
            "--asksrc", help="Ask for sources", action="store_true", default=False
        )
        parser.add_argument(
            "--backend", help="Imaging backend", type=str, default="rascil"
        )
        parser.add_argument(
            "--telescope",
            help="Telescope",
            type=str,
            default="LOFAR, '?' for available telescopes",
        )
        parser.add_argument("--maxflux", help="Max flux", type=float, default=10.0)
        parser.add_argument(
            "--freq", help="Frequency in MHz (default = 1 MHz)", type=float, default=1e6
        )
        parser.add_argument(
            "--fov",
            help="Field of view in degrees (default = 1 degree)",
            type=float,
            default=1.0,
        )
        parser.add_argument(
            "--pixel",
            help="Size of pixel in arcsec (default = 1)",
            type=float,
            default=1.0,
        )
        parser.add_argument(
            "--clean", help="Clean the image", action="store_true", default=False
        )
        parser.add_argument(
            "--relative", help="Relative position", action="store_true", default=False
        )
        ...
        if args.telescope == "?":
            ...
            print(get_args(RASCILTelescopes))
            print("OSKAR:")
            ...
            print(
                get_args(OSKARTelescopesWithVersionType)
                + get_args(OSKARTelescopesWithoutVersionType)
            )
            sys.exit(0)
        ...
        if args.backend == "rascil":
            ...
            if args.telescope in telescope_types:
                ...
                printlog(
                    f"Telescope {args.telescope} not found. Loading LOFAR as default. Values accepted: {telescope_types}",
                    t0,
                )
        ...
        sky_data = []
        printlog(
            f"Starting the simulation. Command line arguments: {' '.join(sys.argv)}", t0
        )
        ...
        if args.cfg:  # load sources from file
            ...
            if args.relative:
                result = calculate_lst(
                    telescope.centre_longitude,
                    telescope.centre_latitude,
                    observation_time_local.strftime("%Y-%m-%d %H:%M:%S"),
                )
                sky_data[:, 0] -= x0
                sky_data[:, 1] -= y0
                x0 = result["RA_zenith"]
            ...
            sky_data[:, 2] = ((sky_data[:, 2] + limit) / (2 * limit)) * (
                args.maxflux - 0.1
            ) + 0.1
            sky_data[:, 0] += x0
            sky_data[:, 1] += y0
        ...
        sky.explore_sky(
            [x0, y0],
            wcs=wcs,
            filename=fout.replace(".png", "_sources.png"),
            xlabel="RA",
            ylabel="DEC",
            vmin=0,
        )
        ...
        visibility_path = os.path.join(
            current_path, "aux.MS"
        )  # path to the visibility file; if you use the WSCLEAN backend, the path must be absolute
        simulation.run_simulation(
            telescope, sky, observation, visibility_path=visibility_path
        )
        ...
        dirty.plot(
            title=f"Dirty image {args.backend.upper()} ({telescope.name.upper()})",
            filename=fout,
            wcs_enabled=True,
            xlabel="RA",
            ylabel="DEC",
        ...
        json_data = {
            "sources": sources,
            "phase_center": {"ra": x0, "dec": y0},
            "observation_date": observation_time_local.strftime("%Y-%m-%d %H:%M:%S"),
        }
        if args.cfg is None:
        ...
        if args.clean:
            ...
            if args.backend in ["rascil", "all"]:
                try:
                    ...
                    restored.plot(
                        title=f"Cleaned image (RASCIL) {args.backend.upper()} ({telescope.name.upper()})",
                        filename=fout.replace(".png", "_cleaned.png"),
                        wcs_enabled=True,
                        xlabel="RA",
                        ylabel="DEC",
                    )
                    printlog(
                        f"Saved cleaned image to {fout.replace('.png', '_cleaned.png')}",
                        t0,
                    )
                    ...
                    config = WscleanImageCleanerConfig(
                        imaging_npixel=imaging_npixel, imaging_cellsize=imaging_cellsize
                    )
                    cleaner = WscleanImageCleaner(config)
                    cleaned = cleaner.create_cleaned_image(
                        visibilities,
                        output_fits_path=fout.replace(".png", "_wccleaned.fits"),
                    )
                    cleaned.plot(
                        title=f"Cleaned image (WSCLEAN) {args.backend.upper()} ({telescope.name.upper()})",
                        filename=fout.replace(".png", "_cleaned.png"),
                        wcs_enabled=True,
                        xlabel="RA",
                        ylabel="DEC",
            ...
            if args.backend in ["oskar", "all", "wsclean"]:
                ...
                printlog(
                    f"Saved cleaned image to {fout.replace('.png', '_cleaned.png')}", t0
                )
    except Exception as e:
        printlog(f"Error: {e}", t0)
        printlog("Exiting...", t0)
        sys.exit(1)
