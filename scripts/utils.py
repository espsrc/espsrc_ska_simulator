...
def mapping_unit(unit_str):
    unit_mapping = {"JY": "Jy", "DEG": "deg", "JY/BEAM": "Jy/beam", "HZ": "Hz"}
    if unit_str is None:
        return None
    ...
    if telescope_name in DIAMETERS:
        ...
        if "SKA" in telescope_name or "SKA1" in telescope_name:
            ...
            if "LOW" in telescope_name:
                ...
                raise ValueError(
                    f"Telescope {telescope_name} not found. Available telescopes: {', '.join(DIAMETERS.keys())}"
                )
        raise ValueError(
            f"Telescope {telescope_name} not found. Available telescopes: {', '.join(DIAMETERS.keys())}"
        )
    ...
    class SkyModel(KaraboSkyModel):
        ...
        def from_json(json_data):
            ...
            try:
                ...
                skyModel.phase_center = SkyCoord(
                    ra=center_ra, dec=center_dec, frame="icrs"
                )
                return skyModel
            except Exception as e:
                print(show_exc(e))
        ...
        def from_fits(
            ...
            if os.path.exists(fits_file):
                ...
                np_samples = np.vstack(
                    (np.array(ra_list), np.array(dec_list))
                ).transpose()
        ...
        def get_center(self, sources=None) -> SkyCoord:
            if self.phase_center is not None:
                ...
                if sources.size > 0:
                    ...
                    raise ValueError(
                        "SkyModel has no sources and phase_center is not set."
                    )
