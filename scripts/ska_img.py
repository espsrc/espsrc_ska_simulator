...
from astropy.coordinates import FK5, Angle, Galactic, SkyCoord
...
from utils import Source, fit_lines_Npts_per_pixel, show_exc
...
def f_p(args):
    ...
    return [
        idx,
        idy,
        coord.icrs.ra.to(u.deg).value,
        coord.icrs.dec.to(u.deg).value,
        value,
    ]


def distance(m, ref, factor=(1.0, 1.0)):
    aux = np.zeros(m.shape)
    for idy, row in enumerate(m):
        for idx, col in enumerate(row):
            aux[idy, idx] = np.sqrt(
                (factor[0] * (idy - ref[0])) ** 2 + ((factor[1] * (idx - ref[1])) ** 2)
            )

    return aux


def distance_shape(m_shape, ref, factor=(1.0, 1.0)):
...
class SKAImage:
        ...
        if not map_mode:
            if len(list_images) == 2:
                ...
                if freq1 is None or freq2 is None:
                    raise Exception(
                        "Cannot compute spectral index: rest frequency not found in one of the images"
                    )

                if img1.data.shape != img2.data.shape:
                    raise Exception(
                        "Cannot compute spectral index: images have different shapes"
                    )
                ...
                list_log_freqs = [
                    np.log10(img.restfreq().to(u.Hz).value) for img in list_images
                ]
                list_log_fluxes = [
                    np.log10(img.janskys().to(u.Jy).value) for img in list_images
                ]
            ...
            alpha_map = fit_lines_Npts_per_pixel(
                list_matrix, list_freqs, mode="loglog", min_positive=1e-30
            )[0]
        ...
        if path is not None:
            if os.path.exists(path):
                ...
                if self.frame != Galactic:
                    self.coords_orig = SkyCoord(
                        ra=(header["CRVAL1"] * u.Unit(header["CUNIT1"])).to(u.deg),
                        dec=(header["CRVAL2"] * u.Unit(header["CUNIT2"])).to(u.deg),
                        frame=self.frame,
                    )
                    self.coords = (
                        self.coords_orig.ra,
                        self.coords_orig.dec,
                        self.coords_orig.frame,
                    )
                else:
                    self.coords_orig = SkyCoord(
                        Galactic(
                            (header["CRVAL1"] * u.Unit(header["CUNIT1"])).to(u.deg),
                            (header["CRVAL2"] * u.Unit(header["CUNIT2"])).to(u.deg),
                        ),
                        frame=Galactic,
                    )
                    self.coords = (
                ...
                # header['NAXIS'] = 2
                self.step = (
                    (header["CDELT1"] * u.Unit(header["CUNIT1"])).to(u.deg),
                    (header["CDELT2"] * u.Unit(header["CUNIT2"])).to(u.deg),
                )
                ...
                self.extent = (
                    x0,
                    x0 + self.step[0].value * self.size[0],
                    y0,
                    y0 + self.step[1].value * self.size[1],
                )
                self.axis = [
                    np.arange(
                        x0, x0 + self.step[0].value * self.size[0], self.step[0].value
                    ),
                    np.arange(
                        y0, y0 + self.step[1].value * self.size[1], self.step[1].value
                    ),
                ]
            else:
            ...
            try:
                ...
                if header is None:
                    ...
                    try:
                        ...
                        xN, yN = self.pix2coords(
                            self.header["NAXIS1"], self.header["NAXIS2"], 0
                        )
                        ...
                        (x0, xN, y0, yN) = (
                            1 * header["CDELT1"],
                            self.size[0] * header["CDELT1"],
                            1 * header["CDELT2"],
                            self.size[1] * header["CDELT2"],
                        )
                    ...
                    correction = header["CDELT2"] / (
                        x0 / 15 * math.cos(math.radians(180))
                    )
                    self.pixelarea = np.abs(self.step[0] * self.step[1])
                    self.axis = [
                        np.arange(x0, x0 + correction * self.size[0], correction),
                        np.arange(
                            y0,
                            y0 + self.step[1].value * self.size[1],
                            self.step[1].value,
                    ...
                    self.coords_orig = SkyCoord(
                        ra=(x0 * unit).to(u.deg), dec=(y0 * unit).to(u.deg)
                    )
                    self.coords = (
                        self.coords_orig.ra,
                        self.coords_orig.dec,
        ...
        if not hasattr(fov, "unit"):
            if isinstance(fov, (int, float)):
                ...
                try:
                    ...
                    raise ValueError(
                        "fov must be an astropy Quantity with angular units (e.g. 1.5deg, 2rad, 30arcmin)"
                    )
            else:
                raise ValueError("fov must be an astropy Quantity with angular units")
        if not fov.unit.is_equivalent(u.deg):
            raise ValueError(
                "fov must be an angular quantity with astropy units (e.g. 1.5deg, 2rad, 30arcmin)"
            )
        if not freq.unit.is_equivalent(u.Hz):
            raise ValueError(
                "freq must be a frequency quantity with astropy units (e.g. 1.4GHz, 150MHz, 3THz)"
            )
    ...
    def reproject(self, img):
        obj = WCS(img.header)
        array, footprint = rp.reproject_interp(
            (self.data, WCS(self.header)), obj, img.data.shape
        )
    ...
    def data_to_pixel(self, verbose=False):
        if self.header["BUNIT"] == "Jy/beam":
            mylog(
                "\tConverting from Jy/beam to Jy/px...",
                flush=True,
                end=" ",
                verbose=verbose,
            )
            self.data = (
                self.data / (self.omega_beam() / self.omega_pix()).value
            )  # Convert Jy/beam to Jy/px
    ...
    def data_to_beam(self, beam=None, verbose=False):
        if self.header["BUNIT"] != "Jy/beam":
            ...
            self.data *= (
                self.omega_beam(beam) / self.omega_pix()
            ).value  # Convert Jy/px to Jy/beam
            self.header["BUNIT"] = "Jy/beam"
            self.units = "Jy/beam"
    ...
    def restfreq(self, unit=u.GHz, default=None):
        ...
        try:
            if "WAVELENG" in self.header.keys():
                return (self.header["WAVELENG"] * units_in_header).to(
                    unit, equivalencies=u.spectral()
                )
            if "RESTFRQ" in self.header.keys():
                return (self.header["RESTFRQ"] * units_in_header).to(
                    unit, equivalencies=u.spectral()
                )
            elif "REFFREQ" in self.header.keys():
                return (self.header["REFFREQ"] * units_in_header).to(
                    unit, equivalencies=u.spectral()
                )
            elif "RESTFREQ" in self.header.keys():
                return (self.header["RESTFREQ"] * units_in_header).to(
                    unit, equivalencies=u.spectral()
                )
            elif "CRVAL3" in self.header.keys():
                return (self.header["CRVAL3"] * units_in_header).to(
                    unit, equivalencies=u.spectral()
                )
            else:
                return default
    ...
    def extent_to_coords(self, frame=None):
        ...
        if self.frame == FK5:
            coords = (
                SkyCoord(ra=x0, dec=y0, frame=self.frame),
                SkyCoord(ra=x0, dec=y1, frame=self.frame),
                SkyCoord(ra=x1, dec=y1, frame=self.frame),
                SkyCoord(ra=x1, dec=y0, frame=self.frame),
            )
        else:
            coords = (
                SkyCoord(l=x0, b=y0, frame=self.frame),
                SkyCoord(l=x0, b=y1, frame=self.frame),
                SkyCoord(l=x1, b=y1, frame=self.frame),
                SkyCoord(l=x1, b=y0, frame=self.frame),
            )
    ...
    def get_beam(self, defaultBeam=None):
        try:
            ...
            if defaultBeam is not None:
                ...
                print(
                    "Warning: Beam information not found in header. Using default beam of %s"
                    % defaultBeam
                )
                return defaultBeam
            else:
                print(
                    "Warning: Beam information not found in header and no default beam provided."
                )
                return None
    ...
    def zoom(self, zoom, in2d=True):
        ...
        if len(zoom) == 2:
            ...
            zoom_coords = SkyCoord(
                ra_center, dec_center, frame=self.frame, unit=(u.hourangle, u.deg)
            )
        ...
        low_left = SkyCoord(
            ra_center + radius_zoom,
            dec_center - radius_zoom,
            frame=self.frame,
            unit=u.deg,
        )
        up_right = SkyCoord(
        ...
        if in2d:
            subimg = orig_data[
                int(min(y0, y1)) : int(max(y0, y1)), int(min(x0, x1)) : int(max(x0, x1))
            ]
        else:
            subimg = orig_data[
                :,
                :,
                int(min(y0, y1)) : int(max(y0, y1)),
    ...
    def integrate_channels_image(
        ...
        for i in range(ny):
            for j in range(nx):
                ...
                umbral_superior = (
                    mediana_espectral[i, j] + sigma_umbral * mad_espectral[i, j]
                )
                umbral_inferior = (
                    mediana_espectral[i, j] - sigma_umbral * mad_espectral[i, j]
                )
                ...
                mascara_lineas = (espectro > umbral_superior) | (
                    espectro < umbral_inferior
                )
    ...
    def draw(
        ...
        if self.data is not None:
            if nan2zero:
                self.data2d[np.isnan(self.data2d)] = 0.0
            ...
            if colorbar:
                ...
                cbar = plt.colorbar(
                    im, ax=ax, extend="both", format=FormatStrFormatter("%.2e")
                )
    ...
    def draw_beam(self, img, color="green"):
        try:
            ...
            (cx, cy) = (
                ...
                * 0.5
                / (self.omega_pix().to(u.arcsec**2).value ** 0.5),
                beam.major.to(u.arcsec).value
                * 0.5
                / (self.omega_pix().to(u.arcsec**2).value ** 0.5),
            )
    ...
    def sigma(self, value=3.0, noise=None):
    ...
    def spw_range(self, unit=u.GHz):
        try:
            ...
            if freq_min is not None:
                ...
                return (
                    freq_min.to(unit, equivalencies=u.spectral()),
                    freq_max.to(unit, equivalencies=u.spectral()),
                )

            if (
                "CRVAL3" in self.header.keys()
                and "CDELT3" in self.header.keys()
                and "NAXIS3" in self.header.keys()
            ):
                freq_min = (
                    self.header["CRVAL3"]
                    - (self.header["CDELT3"] * self.header["CRPIX3"])
                ) * u.Hz
                freq_max = (
                    self.header["CRVAL3"]
                    + (
                        self.header["CDELT3"]
                        * (self.header["NAXIS3"] - self.header["CRPIX3"])
                    )
                ) * u.Hz
    ...
    def fov(self, unit=u.arcsec):
        if self.data is not None:
            ...
            dx = (
                (self.header["NAXIS1"] - self.header["CRPIX1"])
                * self.header["CDELT1"]
                * u.Unit(self.header["CUNIT1"])
            )
            dy = (
                (self.header["NAXIS2"] - self.header["CRPIX2"])
                * self.header["CDELT2"]
    ...
    def new_header(self, hdu=None, beam=None):
        try:
            ...
            if not beam:
                beam = Beam(
                    10 * u.Unit(hdu.header["CUNIT1"]),
                    10 * u.Unit(hdu.header["CUNIT2"]),
                    0 * u.deg,
                )
    ...
    def janskys(
        ...
        """
        if freq is not None:
            freq_factor = (freq.to(u.Hz).value / self.restfreq().to(u.Hz).value) ** (
                alpha_spec
            )
        else:
            freq_factor = 1.0
        ...
        if center is not None:
            ...
            if isinstance(a, u.Quantity):
                a = a.to(u.Unit(self.header["CUNIT1"])) / (
                    self.header["CDELT1"] * u.Unit(self.header["CUNIT1"])
                )
            if isinstance(b, u.Quantity):
                b = b.to(u.Unit(self.header["CUNIT2"])) / (
                    self.header["CDELT2"] * u.Unit(self.header["CUNIT2"])
                )
    ...
    def neg_janskys(self):
        beam = self.get_beam()
        npts = np.count_nonzero(self.data < 0.0)
        mean = np.nanmean(self.data[self.data < 0.0])
        omega_beam = beam.sr.to(u.deg**2)
        omega_pix = (self.header["CDELT1"] * u.Unit(self.header["CUNIT1"])) ** 2
        full_area = omega_pix * npts
        n_beams = full_area / omega_beam
    ...
    def detect_sources(
        self, threshold=5.0, step=1, ratio_beam=1, mask=None, verbose=False
    ):
        try:
            wcs = self.wcs2d
            ...
            pixels_in_beam = math.ceil(
                (beam.sr).to(u.arcsec**2) / (pixel_area.to(u.arcsec**2))
            )
            rms = self.mad(verbose=verbose)
            d = Dendrogram.compute(
                self.data2d,
                min_value=threshold * rms,
                min_delta=step * rms,
                min_npix=pixels_in_beam * ratio_beam,
                wcs=wcs,
            )
            leaves = d.leaves
            sources = []
            metadata = {}
            metadata["data_unit"] = (
                self.header["BUNIT"] if "BUNIT" in self.header.keys() else "Jy/beam"
            )
    ...
    def convolve_to_beam(self, beam):
        ...
        try:
            ...
            if current_beam is None:
                ...
                new_data = convolve_fft(
                    self.data2d, kernel_array, normalize_kernel=True, allow_huge=True
                )
    ...
    def mad(
        ...
        try:
            ...
            try:
                ...
                mylog(
                    "Warning in combine.mad !!!! We can not masked the image",
                    verbose=verbose,
                )
            if is_model:
                data = data[((data > 0.0) & ~(np.isnan(data)))]
            next_mad = stats.mad_std(data, axis=None, ignore_nan=True)
            mad = 1e3
            mad_stack = []
            mad_mask = data < cutoff * mad
            while ((next_mad / mad < limit and counter < iters) or (counter == 0)) and (
                mad_mask.sum() > 0
            ):
                mad = next_mad
    ...
    def mad_mask(self, iters=100, limit=0.99, stack=False, cutoff=2.0, mask=None):
        import warnings
        ...
        try:
            ...
            while (next_mad / mad < limit and counter < iters) or (counter == 0):
                ...
                next_mad = stats.mad_std(
                    data[np.where(data < cutoff * mad)], axis=None, ignore_nan=True
                )
                counter += 1
                mad_stack.append(mad)
                if next_mad <= 0:
    ...
    def draw_source(
        ...
        if isinstance(coord, Source):
            ...
            coord = src.coord
            if a is None:
                a = src.major_axis / 2.0
            if b is None:
                b = src.minor_axis / 2.0
            if pa is None:
                pa = src.pa

        if verbose:
            mylog(
                f"Draw source at {coord.to_string('hmsdms')} a={a}, b={b}, pa={pa}, color={color}, ellipse={ellipse}, alpha={alpha}, label={label}"
            )
        ...
        try:
            ...
            if ellipse and a is not None:
                try:
                    ...
                    frame = pixels.plot(
                        ax=ax, lw=lw, color=color, alpha=alpha, fill=False
                    )
                    # ax.add_patch(frame)
    ...
    def mask_outside_ellipse_fits(
        self,
        center,  # (x0_pix, y0_pix) in pixels OR SkyCoord in the sky
        a,
        b,  # major and minor axes: in pixels (float) or angular units (Quantity, e.g. 30*u.arcsec)
        pa,  # ellipse angle
        ...
        a = np.abs(
            a.to(u.deg).value / self.header["CDELT1"]
            if isinstance(a, u.Quantity)
            else a
        )
        b = np.abs(
            b.to(u.deg).value / self.header["CDELT1"]
            if isinstance(b, u.Quantity)
            else b
        )
...
class DendroSource:
    ...
    def coord(self):
        return SkyCoord(
            ra=self.stats.x_cen * u.deg, dec=self.stats.y_cen * u.deg, frame="icrs"
        )
