    ...
    try:
        ...
        if img is not None and node is not None:
            ...
            if node.level == 0:
                ...
                img.data[~(node.get_mask())] = np.nan
                return img.janskys() - (
                    flux_parent / node.ancestor.get_npix() * node.get_npix()
                )
        else:
            return 0 * u.Jy
    except Exception as e:
...
def process_img(params):
    try:
        ...
        if args.pblim < 1.0:
            ...
            img.data[(img.data == 0.0)] = 8.0 * mad
            img.data[(img.data == 1.0)] = 8.0 * mad
            trunk_color = "blue"
        ...
        d = Dendrogram.compute(
            img.data,
            min_value=args.min * mad,
            min_delta=mad * args.delta,
            min_npix=n_pixels,
        )
        d.save_to(
            "DDRG_{}-{}-dend_delta_{}_min_{}{}.fits".format(
                region_name, project, args.delta, args.min, posfix
            )
        )
        leaves = d.leaves
        ...
        if args.png:
            ...
            axis = img.draw(
                fig,
                exp=args.exp,
                title="{} {} min={:.2f} delta={:.2f} mad={:.4E}".format(
                    img.name, project, args.min, args.delta, mad
                ),
            )
            p = d.plotter()
            ax = fig.get_axes()[0]
            ...
            if args.catalogues:
                for path in args.catalogues:
                    if region_name.split(".")[0] in path:
                        ...
                        for coord in coords:
                            ...
                            if (
                                x < img.data.shape[0]
                                and y < img.data.shape[1]
                                and x >= 0
                                and y >= 0
                            ):
                                counter += 1
                                mylog(
                                    "{:.2f}%\r".format(counter / (len(coords)) * 100),
                                    end="",
                                )
                                axis.show_markers(
                                    coord.ra,
                ...
                mylog(
                    "\n\n{} ({}) sources in {} ({:.2f}%)".format(
                        counter,
                        counter - out_of_region,
                        img.name,
                        counter / (len(coords)) * 100.0,
                    )
                )
                ...
                try:
                    ...
                    if node.level == 0:
                        if node.is_leaf:
                            ...
                            if args.numbers:
                                axis.add_label(
                                    coord_cm.ra.to(u.deg).value,
                                    coord_cm.dec.to(u.deg).value,
                                    f"{node.idx}",
                                    color="blue",
                                )
                            else:
                        ...
                        if node.is_leaf and args.pblim < 1.0:
                            counter += 1
            ...
            if posfix != "":
                ...
                plt.savefig(
                    "{}-{}-dend_mad_{}_pblim_{}_delta_{}_min_{}{}.png".format(
                        region_name,
                        project,
                        args.mad,
                        args.pblim,
        ...
        for idx, node in enumerate(mylist):
            ...
            if node.level == 0:
                if node.is_leaf:
                    cores += 1
                    table1.append(
                        "{}-{}\#TRXX-LV{:03d}  &   {:s} &   {:s} & {:.2f} & {:.2f} & {:.2f}, {:.3f} & {:.2f} & {:.2f} & __DUST__ \\\\ \n".format(
                            region_name,
                            project,
                            node.idx,
                            coord_cm.ra.to_string(
                                unit=u.hour, sep=("hms"), precision=2
                            ),
                            coord_cm.dec.to_string(
                                unit=u.degree, sep=("dms"), precision=2
                            ),
                            area.value,
                            ellip.value,
                            radius,
                            radius_pc,
                            clump_flux(node, img).to(u.mJy).value,
                            stat.flux.to(u.mJy).value,
                        )
                    )
                else:
                    table1.append(
                        "{}-{}\#TR{:02d}  &   {:s} &   {:s} &   {:.2f} & - & {:.2f}, {:.3f} & {:.2f} & {:.2f} & __DUST__ \\\\ \n".format(
                            region_name,
                            project,
                            node.idx,
                            coord_cm.ra.to_string(
                ...
                if node.is_leaf:
                    ...
                    table1.append(
                        "{}-{}\#TR{:02d}-LV{:03d}  &   {:s} &   {:s} &   {:.2f} &   {:.2f} &   {:.2f}, {:.3f}   & {:.2f} & {:.2f} & __DUST__ \\\\ \n".format(
                            region_name,
                            project,
                            node.ancestor.idx,
                            node.idx,
                            ...
                            clump_flux(
                                node,
                                img,
                                clump_list_flux[clump_list.index(node.ancestor)],
                            )
                            .to(u.mJy)
                            .value,
                            stat.flux.to(u.mJy).value,
                    ...
                    table2.append(
                        "{}-{}\#TR{:02d}-LV{:03d}  &   {:s} &   {:s} &   {:.1f} & {:.3f} \\\\ \n".format(
                            ...
                            dist,
                            (img.pixelarea.to(u.arcsec**2) ** (0.5)).value,
                        )
                    )
        ...
        plt.title(
            "{} {} min={:.2f} delta={:.2f}".format(
                img.name, project, args.min, args.delta
            )
        )
        ...
        plt.savefig(
            "{}-{}-dend_delta_{}_min_{}{}_hist.png".format(
                region_name, project, args.delta, args.min, posfix
            )
        )
...
def write_csv(fname, tables):
    ...
    for lines in tables:
        ...
        for idx, line in enumerate(lines):
            ...
            f.write(
                "{}\n".format(
                    ";".join([current_region.split("-")[-1]] + fields + [region_name])
                )
            )
    if f:
        f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--delta", type=float, default=5.0, help="delta * MAD => delta between trunks"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="1kpc",
        help="Distance with units in astropy.units format (default='1kpc')",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Max Size (in pc) for cores. Default = 0.1",
    )
    parser.add_argument(
        "--spec_file", type=str, default=None, help="Text File with spectral index info"
    )
        ...
        with mp.Pool(cpus) as p:
            ...
            if args.spec_file is not None:
                ...
                for line in lines:
                    fields = line.replace("\n", "").split(" ")
                    spec[
                        "{}{}{}".format(fields[0].split("-")[-1], fields[1], fields[2])
                    ] = fields
            lines = []
            lines2 = []
            ...
            headers = []
            headers.append(
                "\\begin{table}[!ht] \\scriptsize \\centering \\begin{tabular}{l l l r r r r r r} \\hline \n"
            )
            for item in results:
                lines.append(item[0])
            ...
            ROW_PER_PAGES = 70
            headers.append(
                "ID              & R.A. & Dec   & Area                 & Ellip & Radius       & ST\_flux & T\_flux & F/D \\\\ \n"
            )
            headers.append(
                " __IDREGION__   &      &       & [arcsec$^2$]  &       & [arcsec, pc] & [mJy]    & [mJy] & \\\\ \n"
            )
            write_tex(
                "table{}.tex".format(posfix), lines, headers, ROW_PER_PAGES, spec=spec
            )

            # lines = sorted(lines2)
            headers = []
            headers.append(
                " \\begin{table}[!ht] \\scriptsize \\centering \\begin{tabular}{l l l l l } \\hline \n"
            )
