...
from utils import SkyModel, show_exc

if __name__ == "__main__":
    try:
        args = argparse.ArgumentParser()
        args.add_argument(
            "--fits_model", type=str, required=True, help="Path to the FITS model file"
        )
        args.add_argument(
            "--output", type=str, help="Path to the output model file", default=None
        )
        args.add_argument(
            "--sigma",
            type=float,
            help="Sigma threshold for source detection",
            default=10.0,
        )
        args.add_argument(
            "--freq",
            type=float,
            help="Frequency in MHz for source fluxes",
            default=None,
        )
        ...
        args.add_argument(
            "--components",
            type=int,
            help="Number of components to use for source modeling",
            default=None,
        )
        args.add_argument(
            "--rms",
            type=float,
            help="RMS noise level in Jy/beam to use for thresholding",
            default=None,
        )
        ...
        if frequency_mhz is None:
            ...
            if "RESTFRQ" in header:
                ...
                "CTYPE3" in header and "FREQ" in header["CTYPE3"] and "CRVAL3" in header
            ):
                frequency_mhz = header["CRVAL3"] / 1e6  # Convert Hz to MHz
            elif (
                "CTYPE4" in header and "FREQ" in header["CTYPE4"] and "CRVAL4" in header
            ):
                frequency_mhz = header["CRVAL4"] / 1e6  # Convert Hz to MHz
            else:
                print(
                    "Frequency not specified and could not be found in FITS header. Please provide --freq argument."
                )
                sys.exit(1)
        ...
        print(
            f"Using threshold value: {threshold_value:.6f}. Pixes above threshold: {np.sum(~np.isnan(img.data))}"
        )
        units = img.get_unit()
        ...
        if "pix" in str(units).lower():
            ...
            units = "Jy"
        oskar_sky = OskarSky.from_fits_file(
            args.fits_model,
            min_abs_val=threshold_value,
            frequency_hz=(frequency_mhz * u.MHz).to(u.Hz).value,
            default_map_units=units,
            override_units=True,
            spectral_index=0,
        )
