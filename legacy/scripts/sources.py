...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument("--telescope", help="Telescope", type=str, default="LOFAR, '?' for available telescopes")
        ...
        parser.add_argument("--clean", help="Clean the image", action="store_true", default=False)
        parser.add_argument("--relative", help="Relative position", action="store_true", default=False)
        ...
        if args.telescope == "?":
            ...
            print (get_args(OSKARTelescopesWithVersionType) + get_args(OSKARTelescopesWithoutVersionType))
            sys.exit(0)
        ...
        dirty.plot(title=f"Dirty image {args.backend.upper()} ({telescope.name.upper()})", filename=fout, wcs_enabled=True, xlabel='RA', ylabel='DEC', vmax=maxflux * 1.05, vmin=0)
        ...
        if args.clean:
            ...
            if args.backend in ["rascil", "all"]:
                try:
                    ...
                    restored.plot(title=f"Cleaned image (RASCIL) {args.backend.upper()} ({telescope.name.upper()})", filename=fout.replace(".png", "_cleaned.png"), wcs_enabled=True, xlabel='RA', ylabel='DEC')
